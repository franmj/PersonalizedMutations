...
import re
import os
from mutations_gather import read_manage_data



def normalize_data(df_mutations,df_reference,sample_id):
    df_final = pd.DataFrame(data=None, columns=df_mutations.columns,index=df_mutations.index)
    df_reference = 1 / df_reference
    for column in df_mutations[sample_id].columns:
        df_reference = 1 / df_reference
        df_final[sample_id,colum] = df_mutations[sample_id,column]*df_reference[sample_id,column]
        # NOrmalized
    
    
    df_final[sample_id] = df_final[sample_id] / df_final[sample_id].sum()
    return df_final
...
def get_Error (tumor, signatures, w):
  w_norm = w/sum(w)
